...
class IoTSystem(metaclass=MetaIoTSystem):
    def __init__(self, timing_exp_list=[], build_ts=True):
            ...
            if '#' in t_exp:
                ...
                for ch_name, ch in self.channel_dict.items():
                    for act_name, act in ch.action_dict.items():
                        ...
                        act_match_set = re.match(r'^(?P<var>\w+)Set(?P<val>\w+)$', act_name)
                        if act_match_numeric:
                            ...
                            var_name = '%s.%s' % (ch_name, act_match_set.group('var'))
                            value = act_match_set.group('val')
                            if var_name in exp:
                                exp_t = exp.replace(var_name, value)
                                if Parser.calcIneq(exp_t):
                                    self.timer_type1_event_l_dict[t_exp].append('%s.%s' % (ch_name, act_name))
    ...
    def getApDict(self, field=None):
        if field:
            self._restoreFromStateVector(field)
        label = self.getLabel()
        ap_dict = dict()
        for value, ap in zip(label, self.getAllAp()):
            ap_dict[ap] = value
        if self._actionTriggered():
            ap_dict['_triggered'] = 1
        else:
            ap_dict['_triggered'] = 0
        return ap_dict
    ...
    def getPossibleEvents(self, prefix, field=None):
        if field:
            self._restoreFromStateVector(field)
        event_list = list()
        for ch_name, channel in self.channel_dict.items():
            if prefix.startswith(ch_name):
                for event_name, event_func in channel.enable_dict.items():
                    if ('%s.%s' % (ch_name, event_name)).startswith(prefix):
                        if event_func(channel):
                            event_list.append('%s.%s' % (ch_name, event_name))
        return event_list

    def resolveTriggeredRules(self):
        event_list = list()
        while self._actionTriggered():
            for tap_name, tap in sorted(self.tap_dict.items()):
                if self.trigger_dict[tap_name]:
                    if not tap.trigger:
                        self.trigger_dict[tap_name] = self._tapConditionSatisfied(tap)
                        ...
                        self.trigger_dict[tap_name] = 0
                    self._applyAction(tap.action)
                    event_list.append(tap.action)
                    break
        return event_list

    def getSmallestTimerValue(self):
        val_type1_l = [val for exp_t, val in sorted(self.timer_type1_dict.items()) if val > 0]
        val_type2_l = [val for exp_t, val in sorted(self.timer_type2_dict.items()) if val > 0]
        val_l = val_type1_l + val_type2_l
        if val_l:
    ...
    def applyAction(self, action, field=None):
        if field:
            self._restoreFromStateVector(field)
        self._applyAction(action)
        return action

    def applyActionBypassChannelModel(self, action, field=None):
        if field:
            self._restoreFromStateVector(field)

        cap_name = action.split('=')[0]
        value = action.split('=')[1]
        channel_name = cap_name.split('.')[0]
        var_name = cap_name.split('.')[1]

        if value in ('true', 'false'):
            # boolean
            self.channel_dict[channel_name].state_dict[var_name] = int(value == 'true')
        elif value.replace('.', '').isnumeric():
            # numeric
            self.channel_dict[channel_name].state_dict[var_name] = \
                type(self.channel_dict[channel_name].state_dict[var_name])(value)
        else:
            # set
            self.channel_dict[channel_name].state_dict[var_name] = value

    def applyTick(self, time=None, field=None):
        if field:
            self._restoreFromStateVector(field)
        target_time = self.getSmallestTimerValue()
        if not time or time >= target_time:
            return self._applyTick()
        else:
            for exp_t, val in self.timer_type1_dict.items():
                if val > 0:
                    self.timer_type1_dict[exp_t] = val - time if val > time else 0
            for exp_t, val in self.timer_type2_dict.items():
                if val > 0:
                    self.timer_type2_dict[exp_t] = val - time if val > time else 0
            return 'tick[%d]' % time
    ...
    def getFieldNameList(self):
        ...
        channel_list = [item for item in sorted(self.channel_dict.items())]
        for channel_name, channel in channel_list:
            state_name_list = [state for state, val in sorted(channel.state_dict.items())]
            for state in state_name_list:
                field_name_list.append('%s.%s' % (channel_name, state))
        return field_name_list

    def resetTimer(self):
        self.timer_type1_dict = {k: 0 for k, v in self.timer_type1_dict.items()}
        for k, v in self.timer_type2_dict.items():
            if all([self._conditionSatisfied(ap) for ap in self.timer_type2_ap_l_dict[k]]):
                self.timer_type2_dict[k] = self.timer_type2_time_dict[k]
            else:
                self.timer_type2_dict[k] = -1


def generateIoTSystem(name, channel_dict, tap_dict, timing_exp_list, build_ts=True):
    class_name = 'Class' + name
    dct = {**channel_dict, **tap_dict}
    TempIoTSystem = MetaIoTSystem(class_name, (IoTSystem,), dct)
    result = TempIoTSystem(timing_exp_list, build_ts=build_ts)
    return result
