...
class Tap(object):
    ...
    def __str__(self):
        if '#' in self.trigger:
            time, trigger = self.trigger.split('#')
            trigger_str = 'it becomes true that %s lasts for more than %s seconds' % (trigger, time)
        elif self.trigger.startswith('clock.clock_time='):
            clock_trig_val = int(self.trigger[17:])
            seconds = clock_trig_val % 60
            clock_trig_val //= 60
            minutes = clock_trig_val % 60
            clock_trig_val //= 60
            hours = clock_trig_val
            trigger_str = 'it becomes %d:%d:%d' % (hours, minutes, seconds)
        else:
            trigger_str = self.trigger
        return 'IF %s WHILE %s, THEN %s' % (trigger_str, self.condition, self.action)
