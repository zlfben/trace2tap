...
FILENAME=$filename FILENAMECORE=$filenamecore docker-compose -f docker-compose-no-build.dev.yml build

if [ "$insertdata" == true ]; then
    docker-compose -f docker-compose-no-build.dev.yml down -v
    docker-compose -f docker-compose-no-build.dev.yml up -d postgres
    docker-compose -f docker-compose-no-build.dev.yml exec postgres sh /home/postgres/init-db.sh
fi

MIGRATION=$migration docker-compose -f docker-compose-no-build.dev.yml up
